...
export default function Chat({ chatRooms }) {
    ...
    const sendMessage = async e => {
        ...
        e.preventDefault();
        // Ensure that the input given by user is valid
        if (input.trim()) {
        ...
        <div className="chat">
            ...
            <div className="chat__header">
                ...
                <div className="chat__headerInfo">
                    <h3>{roomName}</h3>
                    {lastSeen ?
                        <p>Last seen {new Date(lastSeen).toTimeString()}</p> :
            ...
            <div className="chat__body">
                {messages.map(message => (
                    ...
                    <p className={`chat__message ${message.userId === user.uid && "chat__receiver"}`} key={message._id}>
                        ...
                        {message.message}
                        <span className="chat__timestamp">{(lastSeen) && new Date(lastSeen).toTimeString()}</span>
                    </p>
                ))}
            </div>
            ...
            <div className="chat__footer">
                ...
                {/* Switch to Send Icon from MicIcon if any letter is typed in the input box */}
                {input.trim()?
                <IconButton>
